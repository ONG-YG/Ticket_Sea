...
/**
 * Servlet implementation class PromoMgrServlet
 */
@WebServlet(name = "PromoMgr", urlPatterns = { "/promoMgr.do" })
public class PromoMgrServlet extends HttpServlet {
	private static final long serialVersionUID = 1L;
       
    /**
     * @see HttpServlet#HttpServlet()
     */
    public PromoMgrServlet() {
        super();
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		// 2. 비즈니스 로직
		ReservePageData promoPd = new MypageService().promoMgr(currentPage, memberNo);

		if (promoPd != null) {
			// 3. jsp 페이지로 넘겨준다
			RequestDispatcher view = request.getRequestDispatcher("views/mypage/promoMgr.jsp");
			request.setAttribute("promoPd", promoPd);
			view.forward(request, response);

		} else {
			response.sendRedirect("/views/mypage/promoMgrNoData.jsp");
		}
