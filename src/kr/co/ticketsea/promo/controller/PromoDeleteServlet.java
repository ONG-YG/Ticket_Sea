...
/**
 * Servlet implementation class PromoDeleteServlet
 */
@WebServlet(name = "PromoDelete", urlPatterns = { "/promoDelete.do" })
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		try {
			...
			if(userId.equals(writer)) {
				...
				int result = new PromoService().deletePromo(boardP_no,userId);
				
				if(result>0) 
				{
					response.sendRedirect("/views/promo/deleteSuccess.jsp");
				}else {
					throw new Exception();
				}
			}else {
