 ...
 */
@WebServlet(name = "PromoList", urlPatterns = { "/promoList.do" })
public class PromoListServlet extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		//1. 페이징 처리를 위하여 페이지를 저장하는 변수를 생성
				...
				PageData pd = new PromoService().promoAllList(currentPage);
				
				
				//3. 결과값을 view페이지로 리턴
				if(pd!=null)
				{
					RequestDispatcher view = request.getRequestDispatcher("views/promo/promoAllList.jsp");
